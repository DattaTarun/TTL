...
const MentorCard = ({ mentor }) => {
  return (
    <div className="bg-[#FFF5F7] p-6 rounded-lg shadow-xl flex flex-col items-center transition duration-500 hover:scale-110">
      <img src={profilecircle} alt={mentor.name} className="w-24 h-24 rounded-full mb-4 self-center" />
      ...
      <Link to={`/profile/${mentor.name}`}>
        <button className="mt-4 bg-[#821D30] text-white py-2 px-6 rounded-lg font-montserrat hover:bg-gray-600">Connect</button>
      </Link>
    </div>
  );
};
